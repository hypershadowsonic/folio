...
import type { PricePoint, TickerSearchResult, CacheStats, PriceCache } from '@/types'
...
export async function fetchPrices(
  ...
  // 1. Check cache
  const cached = await db.priceCache.get(ticker)
  ...
  try {
    ...
    await db.priceCache.put(newCacheEntry)
 ...
 * Returns partial results — missing tickers have empty arrays (network failure
 * for one ticker does not block the others).
 */
...
/**
 * Fetch USD/TWD exchange rate history.
 * Wrapper around fetchPrices using the Yahoo Finance ticker USDTWD=X.
 */
