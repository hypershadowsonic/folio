...
import { useBuilds, useBenchmarks, useCompares } from '@/db/hooks'
...
export function Builds() {
  ...
  // Consume pendingNav only once the target item has loaded from IndexedDB
  useEffect(() => {
    if (!pendingNav) return
    const exists =
      (pendingNav.kind === 'build' && builds.some((b) => b.id === pendingNav.id)) ||
      (pendingNav.kind === 'benchmark' && benchmarks.some((b) => b.id === pendingNav.id)) ||
      (pendingNav.kind === 'compare' && compares.some((c) => c.id === pendingNav.id))
    if (!exists) return
    setSelectedId(pendingNav.id)
    setView(`detail-${pendingNav.kind}` as BuildsView)
    clearPendingNav()
  }, [pendingNav, builds, benchmarks, compares])
  ...
  async function handleFavoriteToggle(item: ListItem) {
    await db.transaction('rw', db.builds, db.benchmarks, db.compares, async () => {
      ...
      await db.compares.filter((b) => b.isFavorite).modify({ isFavorite: false })
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Backtests, benchmarks, and comparisons</p>
        </div>
          ...
          <CardContent className="flex flex-col items-center justify-center py-12 gap-3 text-center">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground max-w-xs">
                Create a Build to backtest a DCA strategy, a Benchmark to track a single ticker,
                or a Compare to overlay multiple strategies.
              </p>
            </div>
