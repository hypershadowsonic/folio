...
import { useFavorite } from '@/db/hooks'
...
export function Dashboard() {
  ...
  const setPendingNav = useUIStore((s) => s.setPendingNav)

  function handleOpen() {
    if (!favorite) return
    setPendingNav({ id: favorite.item.id, kind: favorite.type })
    setActiveTab('builds')
  }
  ...
  if (favorite.type === 'build' || favorite.type === 'benchmark') {
    ...
    const itemName = favorite.type === 'build' ? item.name : `${item.ticker} — ${item.name}`
