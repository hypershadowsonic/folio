...
export function runBacktest(
  ...
  // Tracks the most recent price seen for each ticker across all DCA dates.
  // Used as a fallback for valuation when a ticker has no price on the current date
  // (e.g. different trading calendar, data lag, holiday) but holds accumulated shares.
  const lastKnownPriceMap: Record<string, number> = {}
  ...
  for (const dateStr of dcaDates) {
    ...
    for (const h of holdings) {
      ...
      if (price !== null) {
        priceMap[h.ticker] = price
        lastKnownPriceMap[h.ticker] = price  // update last-known for valuation fallback
      }
    }
    ...
    // 'none' always uses proportional buy regardless of triggers.
    if (rebalanceStrategy === 'none') {
    ...
    // Use lastKnownPriceMap as fallback: if a ticker has no price today (different
    // trading calendar, data gap) but has accumulated shares, value them at the most
    // recent known price rather than treating them as worthless.
    const portfolioValue = holdings.reduce((sum, h) => {
      const price = priceMap[h.ticker] ?? lastKnownPriceMap[h.ticker] ?? 0
      return sum + toDcaCurrency(h.shares * price, h.currency, dcaCurrency, fxRate)
    }, 0)
    ...
    timeSeries.push({
      ...
      rebalanceTriggered: triggered && rebalanceStrategy !== 'none',
      rebalanceType: triggered && rebalanceStrategy !== 'none' ? rebalanceStrategy : undefined,
    })
  }
