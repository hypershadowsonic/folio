...
import { useTickerSearch, fetchMultiplePrices, fetchFxRate } from '@/services/yahooFinance'
...
export function BuildCreator({ onDone, onCancel, editBuild }: BuildCreatorProps) {
  ...
  const step4Valid = form.rebalanceStrategy === 'none' || form.rebalanceTriggers.length > 0
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <RadioGroup
          ...
          onValueChange={(v) => setForm((f) => ({ ...f, rebalanceStrategy: v as BuildForm['rebalanceStrategy'] }))}
          className="space-y-2"
        >
          ...
          <div className="flex items-start gap-3 p-3 rounded-lg border has-[:checked]:border-primary has-[:checked]:bg-primary/5 transition-colors">
            <RadioGroupItem value="none" id="strategy-none" className="mt-0.5" />
            <Label htmlFor="strategy-none" className="cursor-pointer">
              <span className="font-medium">No rebalancing</span>
              <p className="text-xs text-muted-foreground font-normal mt-0.5">
                Buy proportional to target allocation — ignore drift entirely
              </p>
            </Label>
          </div>
        </RadioGroup>
      ...
      {/* Triggers — hidden when no rebalancing */}
      {form.rebalanceStrategy !== 'none' && <div className="space-y-2">
