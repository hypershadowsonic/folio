...
import { useLinksForBuild } from '@/db/buildHooks'
import { PromoteToPortfolio } from './PromoteToPortfolio'
import type { Build, BacktestDataPoint } from '@/types'
...
function CustomTooltip({ active, payload, label, currency }: TooltipProps) {
  ...
  const portfolioItem = payload.find((p) => p.dataKey === 'portfolioValue')
  const costItem = payload.find((p) => p.dataKey === 'costBasis')
...
export function BuildDetail({ build, onBack, onEdit, onDelete, onDuplicate }: BuildDetailProps) {
  const result = build.lastBacktestResult
  const [dcaVisible, setDcaVisible] = useState(50)
  const promotionLinks = useLinksForBuild(build.id)

  // Chart data: map timeSeries to plain objects for Recharts
  ...
  const currency = build.dcaCurrency
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 p-4 border-b">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-lg font-bold truncate">{build.name}</h1>
          <p className="text-xs text-muted-foreground">
            {build.holdings.length} holdings · {build.dcaFrequency} {formatCurrency(build.dcaAmount, build.dcaCurrency)}
          </p>
        </div>
        <PromoteToPortfolio build={build} />
      ...
      {/* Lineage badges */}
      {(promotionLinks.length > 0 || build.sourceInfo) && (
        <div className="px-4 pb-1 space-y-1">
          {promotionLinks.length > 0 && (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-muted/50 border text-xs text-muted-foreground">
              <ArrowUpRight className="h-3.5 w-3.5 shrink-0" />
              <span>Promoted to Portfolio</span>
            </div>
          )}
          {build.sourceInfo && (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-muted/50 border text-xs text-muted-foreground">
              ...
              <span>
                Forked from Portfolio ·{' '}
                {build.sourceInfo.forkType === 'target_allocation' ? 'Target Allocation' : 'Historical Snapshot'}
                {build.sourceInfo.snapshotDate && ` (${new Date(build.sourceInfo.snapshotDate).toLocaleDateString()})`}
              </span>
            </div>
          )}
        </div>
        ...
        {!result ? (
          ...
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12 gap-3 text-center">
              ...
              <p className="text-xs text-muted-foreground">Edit the build and run a backtest to see results.</p>
              <Button onClick={onEdit} size="sm">Run Backtest</Button>
            </CardContent>
          </Card>
        ) : (
          <>
            ...
            <div className="grid grid-cols-2 gap-3">
              <MetricCard
                label="Total Return"
                value={fmtPct(s?.totalReturnPct ?? null)}
                valueClass={
                  (s?.totalReturnPct ?? 0) >= 0
              ...
              <MetricCard
                ...
                value={fmtPct(s?.annualizedReturnPct ?? null)}
                valueClass={
                  (s?.annualizedReturnPct ?? 0) >= 0
                    ? 'text-green-600 dark:text-green-400'
                    : 'text-red-600 dark:text-red-400'
                }
              />
              <MetricCard
                label="End Value"
                value={s ? formatCurrency(s.endValue, currency) : '—'}
              />
              <MetricCard
                label="Total Invested"
                value={s ? formatCurrency(s.totalInvested, currency) : '—'}
              />
              <MetricCard
                label="Max Drawdown"
                value={fmtPct(s?.maxDrawdownPct ?? null)}
            ...
            {/* YoY / MoM */}
            {s && (s.yoyGrowthPct !== null || s.momGrowthPct !== null) && (
              <div className="grid grid-cols-2 gap-3">
                {s.yoyGrowthPct !== null && (
                  <MetricCard
                    label="Year-over-Year"
                    value={fmtPct(s.yoyGrowthPct)}
                    valueClass={
                      (s.yoyGrowthPct ?? 0) >= 0
                        ? 'text-green-600 dark:text-green-400'
                        : 'text-red-600 dark:text-red-400'
                    ...
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="border-b">
                          ...
                          <th className="text-right py-1.5 px-2 text-muted-foreground font-medium">Cost (est.)</th>
                          <th className="text-right py-1.5 px-2 text-muted-foreground font-medium">Alloc %</th>
                          <th className="text-right py-1.5 pl-2 text-muted-foreground font-medium">Target %</th>
                        </tr>
                      </thead>
                      <tbody>
                        {lastPoint.holdings.filter((h) => h.shares > 0).map((h) => {
                          const target = build.holdings.find((bh) => bh.ticker === h.ticker)?.targetAllocationPct ?? 0
                          ...
                          return (
                            <tr key={h.ticker} className="border-b last:border-0">
                              ...
                              <td className="py-1.5 px-2 text-right tabular-nums text-muted-foreground">{formatCurrency(estCost, currency)}</td>
                              <td className="py-1.5 px-2 text-right tabular-nums">{h.allocationPct.toFixed(1)}%</td>
                              <td className="py-1.5 pl-2 text-right tabular-nums text-muted-foreground">{target.toFixed(1)}%</td>
                            </tr>
                          )
                        })}
                      </tbody>
            ...
            <p className="text-sm text-muted-foreground text-center pb-2">
              {s?.totalRebalances ?? 0} rebalance{(s?.totalRebalances ?? 0) !== 1 ? 's' : ''} over the period
              {' · '}
              {build.rebalanceStrategy === 'soft' ? 'soft (buy-only)' : build.rebalanceStrategy === 'hard' ? 'hard (sell + buy)' : 'no rebalancing'}
            </p>
          </>
        )}
      </div>
